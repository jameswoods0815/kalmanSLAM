...
class slam:
    def __init__(self,
        ...
        if observation_noise_conv is None:
            observation_noise_conv=5e-3*np.eye(4)
        #IMU sensing nosie 6 demsion
        if imu_noise_conv is None:
            imu_noise_conv=1e-3 *np.eye(6)

        #robot pose to the world
        self.w_T_car=np.eye(4)
        self.imu_noise_conv=imu_noise_conv
        ...
        self.totalObservationSigma=np.block([[prior_pose_conv, np.zeros((6, 3 * num_landmarks))],
                           [np.zeros((3 * num_landmarks, 6)),
                            np.kron(np.eye(num_landmarks), prior_landmark_conv)]])
    ...
    def predict(self,u,tau):
        ...
        tmp1 = scipy.linalg.expm(-tau * getSkew_6x6(u))

        # update the predict sigma for imu:
        self.totalObservationSigma[:6,:6]=tmp1@self.totalObservationSigma[:6,:6]
        self.totalObservationSigma[:6,:6]=self.totalObservationSigma[:6,:6]@tmp1.T

        self.totalObservationSigma[:6,:6]+=self.imu_noise_conv
    ...
    def init_landmarks(self, observation, datamask):
        ...
        if data.size > 0:
            ...
            worldCoor[1, :] = (observation[1, :] - Ks[1, 2]) * (-Ks[2, 3]) / (
                        Ks[1, 1] * (observation[0, :] - observation[2, :]))
    ...
    def get_H(self, datamap):
        ...
        H=np.zeros((num_observe*4,n_update*3+6))

        cam_T_world=np.linalg.inv(self.world_T_cam())
        cam_T_imu=np.linalg.inv(self.imu_T_cam)
        imu_T_world=np.linalg.inv(self.w_T_car)

        for i in range(num_observe):
            tmp=datamap[i]
            H[i*4:(i+1)*4, 6+tmp*3:6+(tmp+1)*3]=self.Ks@diff_func_pi(cam_T_world@mapPostion[i,:].reshape(-1,1))@cam_T_world@P.T

        for i in range(num_observe):
            H[i*4:(i+1)*4,:6]=self.Ks@diff_func_pi(cam_T_world@mapPostion[i,:].reshape(-1,1))@cam_T_imu@circleDot(
                imu_T_world@mapPostion[i,:].reshape(-1,1))
        return H

    def get_postion_and_map_total_simga(self):
        n_updates=self.init_maxid
        postion=self.mapPosition[:n_updates,:]
        conv=self.totalObservationSigma[:6+n_updates*3,:6+n_updates*3]
        return postion, conv
    ...
    def update_value_postion_and_sigma(self,postion,sigma):
        ...
        self.totalObservationSigma[:6+n_updates*3,:6+n_updates*3]=sigma
    ...
    def update(self,observation):
        ...
        if datamap.size>0:
            ...
            invt=np.linalg.inv(H@sigma@H.T+V)
            Kt=sigma@H.T@invt
            mean=Kt@(obsevReal-predict_observe)

            position+=mean[6:,0].reshape(-1,3)
            finalSigma=(np.eye(Kt.shape[0])-Kt@H)@sigma

            self.w_T_car=self.w_T_car @ scipy.linalg.expm(-getSkewMatrix(mean[:6,0]))

            xm=np.linalg.inv(self.w_T_car)
